...
SOURCE_DIR = ROOT / "notebooks" / "Lab 1-1 Load Documents Using LangChain for Different Sources"
PDF_PATH = (
    ROOT
    / "notebooks"
    / "Lab 3-2 Construct a QA Bot that Leverages the LangChain and LLM to Answer Questions from Loaded Documents"
...
def load_text_file():
    return TextLoader(str(SOURCE_DIR / "new-Policies.txt")).load()
...
def load_markdown_file():
    return UnstructuredMarkdownLoader(str(SOURCE_DIR / "markdown-sample.md")).load()


def load_json_file():
    json_path = SOURCE_DIR / "facebook-chat.json"
    print("\nRaw JSON sample:")
...
def load_csv_file():
    return CSVLoader(file_path=str(SOURCE_DIR / "mlb-teams-2012.csv")).load()


def load_csv_as_html_table():
    return UnstructuredCSVLoader(
        file_path=str(SOURCE_DIR / "mlb-teams-2012.csv"),
        mode="elements",
    ).load()


def load_docx_file():
    return Docx2txtLoader(str(SOURCE_DIR / "file-sample.docx")).load()


def load_unstructured_files():
    files = [
        str(SOURCE_DIR / "markdown-sample.md"),
        str(SOURCE_DIR / "new-Policies.txt"),
    ]
